...
if (isBeta) {
      ...
      $(".item-img + .item-name").each(function (index, element) {
        ...
        if (!hasSearchHelper(element)) {
          let extras = {
            ...
            wearable:
              $(element).parent().find(":contains('wearable')").length > 0,
            tradeable:
              $(element).parent().find(":contains('(no trade)')").length === 0,
            itemid: -1,
          };
  ...
  if (inURL("games/teatime")) {
    taviQuest();
    $(document).ajaxSuccess(taviQuest);
  }
  ...
  // Main Shops
  if (
    document.URL.includes("objects.phtml?") &&
    document.URL.includes("type=shop")
  ) {
    $("img[src*='/items/']")
      .parent()
      .parent()
  ...
  if (document.URL.includes("inventory")) {
    $("img[src*='/items/']").each(function (k, v) {
      ...
      if ($nametd.find("hr").exists()) {
        extras.tradeable = !$nametd
          .find("span:contains('(no trade)')")
          .exists();
  ...
  if (isSDB || isCloset) {
    $("img[src*='/items/']").each(function (k, v) {
      ...
      let isWearable =
        isCloset ||
        $(v).parent().parent().find("td").eq(1).text().includes("(wearable)");
      let isNeoCash = false;
      if (isSDB) {
  ...
  // illusen & jhudora
  if (
    $("img[src*='ef_2.gif']").exists() ||
    $("img[src*='darkfaeriequest2.gif']").exists()
  ) {
    let itemname = $("center:contains('Where is my') > b").text();
    $("center:contains('Where is my')")
      .parent()
      .find("img[src*='/items/']")
      .after(makelinks(itemname));
  }
